...
export async function login(req, res) {
  try {
    ...
    if (!process.env.JWT_SECRET_KEY) {
      ...
      return res
        .status(500)
        .json({ message: "JWT Secret key missing in environment config." });
    }
    // console.log("JWT_SECRET_KEY (login):", process.env.JWT_SECRET_KEY);
...
export async function onboard(req, res) {
  ...
  try {
    ...
    //update the user info into the STREAM!
    try {
        await upsertStreamUser({
        ...
        console.log("Error updating Stream user during updating",error.message);
        res.status(500).json({message: "Internal Server Error"})
